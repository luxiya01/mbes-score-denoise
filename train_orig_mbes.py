...
def validate(it, dset, store_results=False, ckpt_path=None):
    ...
    if store_results:
        ...
        os.makedirs(f'{ckpt_path}_results', exist_ok=True)
    ...
    for i, data in enumerate(tqdm(dset, desc='Validate')):
        ...
        if store_results:
            ...
            np.savez(f'{ckpt_path}_results/patch_based_denoise_{i}_patchid_{idx}.npz', pcl_denoised=pcl_denoised, pcl_clean=pcl_clean,
    ...
    # compute F1-score
    binary_metrics['f1_score'] = 2 * (binary_metrics['precision'] * binary_metrics['recall']) / (binary_metrics['precision'] + binary_metrics['recall'])
    ...
    logger.info(f'[Val] Iter {it:4d} Outlier | Accuracy {binary_metrics["accuracy"]*100:.2f}% | Precision {binary_metrics["precision"]*100:.2f}% | Recall {binary_metrics["recall"]*100:.2f}% | F1-score {binary_metrics["f1_score"]:.6f}')
